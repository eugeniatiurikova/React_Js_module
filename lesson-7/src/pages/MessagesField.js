...
const MessagesField = ({ handleSubmit, messages, msgauthor, setMsgauthor, messagetext, setMessagetext, deleteMessage }) => (
    ...
    >
        ...
        <Box className="messagetext">
            <Typography variant="h5" color="primary">Сообщения</Typography>
            {messages.map(item => {
                return (
                    <Card sx={{ margin: '20px 0 20px 0', backgroundColor: 'background.paper' }} key={item.id}>
                        <CardContent>
                            <Stack sx={{ margin: '0 0 10px 0' }}
                                ...
                                spacing={1}
                            ><Typography variant="h6" color="primary">{item.author}:</Typography><IconButton size="small" onClick={() => deleteMessage(item.chatid)}><DeleteOutlinedIcon /></IconButton></Stack>
